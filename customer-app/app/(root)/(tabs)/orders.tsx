...
const OrdersPage = () => {
  ...
  const fetchData = async (pageNumber) => {
    if (pageNumber === 1) {
      ...
      setMoreDataLoading(true); // Start loading
    }
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/api/order/pagination-orders/${user._id}?page=${pageNumber}&limit=10&activeTab=${activeTab}`
      );
      ...
      if (pageNumber === 1) {
        ...
        setMoreDataLoading(false); // End loading
      }
    }
  };
